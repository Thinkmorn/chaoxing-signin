...
export const getLocalUsers = () => {
  // 从 env.json 读取备注
  let envRemarks: Record<string, string> = {};
  try { const env = getJsonObject('env.json'); (env.accounts || []).forEach((a: any) => { if (a.remark) envRemarks[a.phone] = a.remark; }); } catch {}
  ...
  for (let i = 0; i < data.users.length; i++) {
    ...
    arr.push({
      title: envRemarks[user.phone] || user.remark || user.phone,
      value: i,
    });
  }
